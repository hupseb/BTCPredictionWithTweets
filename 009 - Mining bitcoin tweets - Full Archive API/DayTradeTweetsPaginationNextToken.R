...
repeat{
  ...
  res <- POST(url = "https://api.twitter.com/1.1/tweets/search/fullarchive/LIVE.json",
                    ...
                    body = list(query = q, maxResults = max, fromDate = from, toDate = to, "next" = nexttoken), encode = "json")
  }else{
    res <- POST(url = "https://api.twitter.com/1.1/tweets/search/fullarchive/LIVE.json",
                add_headers("authorization" = bearerTokenb, "content-Type" = "application/json"),
                body = list(query = q, maxResults = max, fromDate = from, toDate = to), encode = "json")    
  }
