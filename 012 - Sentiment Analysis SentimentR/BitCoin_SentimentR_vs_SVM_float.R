install.packages(c("ggplot2", "e1071", "caret", "quanteda", "irlba", "randomForest", "jsonlite", "RODBC", "wordcloud", "sentimentr", "magrittr", "dplyr", "pacman"))
install.packages(c("sentimentr"))
library("RODBC")
library("devtools")
library("sentimentr")
library("magrittr")
options(scipen = 999)

# get stored tweets from db and explore in RStudio.
conn <- odbcDriverConnect('driver={SQL Server};server=localhost\\SQLEXPRESS;database=BitcoinTwts;trusted_connection=true')
svmsentiment.raw <- sqlQuery(conn, "SELECT TOP (1000) A.[status_id]
      , B.[created_at]
      , B.[user_id]
      , A.[tweet]
      , A.[svm_sentiment]
      , A.[decisionValue]
      , A.[pos_prob]
      , A.[neg_prob]
FROM [BitcoinTwts].[dbo].[Final_SVM_sentiment] AS A
INNER JOIN [BitcoinTwts].[dbo].[BitcoinTweetsUnion_Filtered_FinalInterval] AS B ON A.status_id = B.status_id
ORDER BY NEWID()", as.is = TRUE)

#-------------------------------------------------------------------------------------------------------------

svmsentiment.sentenced <- svmsentiment.raw %>% 
  get_sentences(svmsentiment.raw$tweet)

senticonv <- function(x) {
  switch(x,
         'positive' = 1,
         'neutral' = 0,
         'negative' = -1,
         as.character(x)
  )
}

accuracy <- function(sa, sb) {
  if ((sa > 0 && sb > 0) || (sa < 0 && sb < 0) || (sa == 0 && sb == 0)) {
    return(1)
  }
  else {
    return(0)
  }
}

## Make syuzhet dictionaries into sentimentr keys
#nrc <- data.frame(
#  words = rownames(syuzhet:::nrc),
#  polarity = syuzhet:::nrc[, "positive"] - syuzhet:::nrc[, "negative"],
#  stringsAsFactors = FALSE
#) %>%
#{as_key(.[.[["polarity"]] != 0, ])}

#nrc -> 2686
#bing -> 3371
#afinn -> 3396
#syuzhet_dict -> 2700
#vadar -> 3122
#inquirer -> 2882
#loughran_mcdonald -> 3167
#senticnet -> 1530
#sentiword -> 1752

#polarity_dt = lexicon::hash_sentiment_jockers,
#valence_shifters_dt = lexicon::hash_valence_shifters,
#hyphen = "",
#amplifier.weight = 0.8
#n.before = 5
#n.after = 2,
#question.weight = 1, adversative.weight = 0.85, missing_value = 0, ...)


dict <- lexicon::hash_sentiment_huliu
#dict <- as_key(syuzhet:::afinn)
#dict <- lexicon::hash_sentiment_loughran_mcdonald
#dict <- lexicon::hash_sentiment_vadar
#dict <- lexicon::hash_sentiment_inquirer
#dict <- as_key(syuzhet:::syuzhet_dict)
#dict <- lexicon::hash_sentiment_nrc
#dict <- lexicon::hash_sentiment_sentiword
#dict <- lexicon::hash_sentiment_senticnet


sentimentr <- function() sentiment(svmsentiment.sentenced,
                                             polarity_dt = dict,
                                             valence_shifters_dt = lexicon::hash_valence_shifters,
                                             amplifier.weight = 0.8,
                                             n.before = 5,
                                             n.after = 2,
                                             question.weight = 0,
                                             adversative.weight = 0.85
                                             )

dat <- sentimentr()
fullaggregated <- aggregate(list(sentimentr_dict = dat$sentiment), by=list(element_id=dat$element_id), FUN=sum)
aggregated <- aggregate(sentiment ~ element_id + status_id + created_at + user_id + svm_sentiment + decisionValue, dat, sum)
aggregated$decisionValue <- as.numeric(aggregated$decisionValue)
View(merge(fullaggregated, dat))

aggregated$svm_sentiment_val <- sapply(aggregated$svm_sentiment, senticonv)
#aggregated$svm_sentiment_val <- as.numeric(aggregated$decisionValue)
aggregated$acc <- mapply(accuracy,aggregated$sentiment,aggregated$svm_sentiment_val)
aggregated <- aggregated[order(aggregated$decisionValue, decreasing=FALSE),]
View(aggregated)
#aggregated <- subset(aggregated, abs(decisionValue) > 1)

#accuracy
sum(aggregated$acc)

library("ggplot2")
ggplot(aggregated) +
  geom_col(width=0.1, aes(x = reorder(aggregated$element_id, -sentiment), y = svm_sentiment_val, fill = "CF", colour = "CF")) +
  geom_col(width=0.1, aes(x = reorder(aggregated$element_id, -sentiment), y = sentiment, fill = "SM",  colour = "SM")) + 
  scale_colour_manual("", 
                      breaks = c("CF", "SM"),
                      values = c("#7BADD7", "#29567D"),
                      guide = FALSE) +
  scale_fill_manual("sources:", 
                      breaks = c("CF", "SM"),
                      values = c("#7BADD7", "#29567D"),
                      labels = c("CrowdFlower", "SentimentR")) +
  ylim(-2, 2) +
  geom_hline(yintercept=0) + 
  geom_vline(xintercept=2364) +
  theme(axis.text.x = element_blank(), axis.ticks.x = element_blank()) + 
  labs(x = "tweets", y = "sentiment") 

ggsave("CrowdFlowerVsSentimentR.png", plot = last_plot(), limitsize = FALSE, scale = 2.5, width=15, height=7, dpi=200, units = "cm", device = "png")
ggsave("CrowdFlowerVsSentimentR12x8.png", plot = last_plot(), limitsize = FALSE, scale = 2.5, width=12, height=8, dpi=200, units = "cm", device = "png")
