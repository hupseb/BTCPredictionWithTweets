...
rfsentiment.raw <- sqlQuery(conn, "SELECT TOP (1000) A.[status_id]
      , B.[created_at]
      , B.[user_id]
      , A.[tweet]
      , A.[rf_sentiment]
      ...
      , A.[pos_prob]
      , A.[posneg]
FROM [BitcoinTwts].[dbo].[Final_RF_sentiment] AS A
INNER JOIN [BitcoinTwts].[dbo].[BitcoinTweetsUnion_Filtered_FinalInterval] AS B ON A.status_id = B.status_id", as.is = TRUE)

#-------------------------------------------------------------------------------------------------------------

rfsentiment.sentenced <- rfsentiment.raw %>% 
  get_sentences(rfsentiment.raw$tweet)
...
sentimentr <- function() sentiment(rfsentiment.sentenced,
                                             polarity_dt = dict,
                                             valence_shifters_dt = lexicon::hash_valence_shifters,
                                             amplifier.weight = 0.8,
                                             n.before = 5,
...
aggregated <- aggregate(sentiment ~ element_id + status_id + created_at + user_id + rf_sentiment, dat, sum)
View(merge(fullaggregated, dat))

aggregated$rf_sentiment_val <- sapply(aggregated$rf_sentiment, senticonv)
aggregated$acc <- mapply(accuracy,aggregated$sentiment,aggregated$rf_sentiment_val)
...
library("ggplot2")
ggplot(aggregated) +
  geom_col(width=0.1, aes(x = reorder(aggregated$element_id, -sentiment), y = rf_sentiment_val, fill = "CF", colour = "CF")) +
