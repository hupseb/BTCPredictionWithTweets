...
aggregated <- aggregate(sentiment ~ element_id + status_id + created_at + user_id + rf_sentiment + posneg, dat, sum)
View(merge(fullaggregated, dat))

#aggregated$rf_sentiment_val <- sapply(aggregated$rf_sentiment, senticonv)
aggregated$rf_sentiment_val <- as.numeric(aggregated$posneg)
