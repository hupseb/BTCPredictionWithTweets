...
positive_all = sqlQuery(conn, "SELECT TOP 500 [tweet]
FROM dbo.CrowdFlower AS TESTDATA
                 ...
                 ORDER BY NEWID()"
                 , as.is = TRUE)
positive = positive_all[1:450,]
positive_test = positive_all[451:500,]

negative_all = sqlQuery(conn, "SELECT TOP 500 [tweet]
FROM dbo.CrowdFlower AS TESTDATA
                 WHERE (relevant_yn LIKE 'yes') AND (relevant_yn_confidence = 1) AND (sentiment_confidence  = 1) AND (sentiment LIKE 'negative') 
                 ORDER BY NEWID()"
                       , as.is = TRUE)
negative = negative_all[1:450,]
negative_test = negative_all[451:500,]
...
container = create_container(mat, as.numeric(sentiment_all),
                             trainSize=1:900, testSize=901:1000,virgin=FALSE) #可以设置removeSparseTerms
...
table(as.numeric(as.numeric(sentiment_all[901:1000])), results[,"SVM_LABEL"])
recall_accuracy(as.numeric(as.numeric(sentiment_all[901:1000])), results[,"SVM_LABEL"])
confusionMatrix(table(as.numeric(as.numeric(sentiment_all[901:1000])), results[,"SVM_LABEL"]))
