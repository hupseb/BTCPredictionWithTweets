...
positive_all = sqlQuery(conn, "SELECT TOP 90 [tweet]
FROM dbo.CrowdFlower AS TESTDATA
                 ...
                 ORDER BY NEWID()"
                 , as.is = TRUE)
positive = positive_all[1:80,]
positive_test = positive_all[81:90,]

negative_all = sqlQuery(conn, "SELECT TOP 90 [tweet]
FROM dbo.CrowdFlower AS TESTDATA
                 WHERE (relevant_yn LIKE 'yes') AND (relevant_yn_confidence = 1) AND (sentiment_confidence = 1) AND (sentiment LIKE 'negative') 
                 ORDER BY NEWID()"
                       , as.is = TRUE)
negative = negative_all[1:80,]
negative_test = negative_all[81:90,]
...
table(as.numeric(as.numeric(sentiment_all[161:180])), results[,"SVM_LABEL"])
recall_accuracy(as.numeric(as.numeric(sentiment_all[161:180])), results[,"SVM_LABEL"])
confusionMatrix(table(as.numeric(as.numeric(sentiment_all[161:180])), results[,"SVM_LABEL"]))
