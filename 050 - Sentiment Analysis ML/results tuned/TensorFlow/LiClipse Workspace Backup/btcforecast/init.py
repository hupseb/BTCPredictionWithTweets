...
def fit_model(model, X_train, Y_train, batch_num, num_epoch, val_split, cbacks):
    """
    ...
    model.fit(X_train, Y_train, batch_size= batch_num, epochs=num_epoch, validation_split= val_split, callbacks=[cbacks])
...
X_train, Y_train, X_test, Y_test, Y_daybefore, unnormalized_bases, window_size = load_data("comb.csv", 50)
print(X_train.shape)
print(Y_train.shape)
print(X_test.shape)
print(Y_test.shape)
...
tbCallBack = TensorBoard(log_dir='C:/logs/btcforecast', histogram_freq=5, write_graph=True, write_images=True)

model, training_time = fit_model(model, X_train, Y_train, 1024, 100, .05, tbCallBack)
