...
positive_all = sqlQuery(conn, paste("SELECT TOP 7000 [tweet], [sentiment], [_unit_id], [status_id]
...
positive <- positive_all[1:6600,1:2]
positive_test <- positive_all[6601:7000,1:2]

negative_all = sqlQuery(conn, paste("SELECT TOP 4100 [tweet], [sentiment], [_unit_id], [status_id]
FROM [BitcoinAnalysis].[dbo].[05. FigureEight_raw] AS TESTDATA
...
negative <- negative_all[1:3700,1:2]
negative_test <- negative_all[3701:4100,1:2]
...
model <- svm(x = container@training_matrix,
...
table(as.numeric(as.factor(tweet_all[10301:11100,2])),results[,"FORESTS_LABEL"])
recall_accuracy(as.numeric(as.factor(tweet_all[10301:11100,2])),results[,"FORESTS_LABEL"])
confusionMatrix(as.numeric(as.factor(tweet_all[10301:11100,2])),results[,"FORESTS_LABEL"])
