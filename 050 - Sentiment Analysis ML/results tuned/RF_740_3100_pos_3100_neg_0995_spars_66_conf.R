...
mtryStart <- 114
bestmtry <- tuneRF(x = as.matrix(container@training_matrix), y = container@training_codes, mtryStart, ntreeTry=200, stepFactor=0.9, improve=0.01, plot=TRUE, trace=TRUE)
print(bestmtry)
...
table(as.numeric(as.factor(tweet_all[(((tr_end-tr_start+1)*2)+1):(nrow(tweet_all)),2])),results[,"FORESTS_LABEL"])
recall_accuracy(as.numeric(as.factor(tweet_all[(((tr_end-tr_start+1)*2)+1):(nrow(tweet_all)),2])),results[,"FORESTS_LABEL"])
confusionMatrix(as.numeric(as.factor(tweet_all[(((tr_end-tr_start+1)*2)+1):(nrow(tweet_all)),2])),results[,"FORESTS_LABEL"])
