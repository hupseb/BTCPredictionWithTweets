...
positive_all = sqlQuery(conn, paste("SELECT TOP 4100 [tweet], [sentiment], [_unit_id], [status_id]
...
positive <- positive_all[1:3700,1:2]
positive_test <- positive_all[3701:4100,1:2]
...
table(as.numeric(as.factor(tweet_all[7401:8200,2])),results[,"FORESTS_LABEL"])
recall_accuracy(as.numeric(as.factor(tweet_all[7401:8200,2])),results[,"FORESTS_LABEL"])
confusionMatrix(as.numeric(as.factor(tweet_all[7401:8200,2])),results[,"FORESTS_LABEL"])
