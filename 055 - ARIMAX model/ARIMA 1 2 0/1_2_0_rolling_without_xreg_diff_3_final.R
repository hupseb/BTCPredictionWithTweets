...
for (i in 2:ncol(btc_ts)) {
  ...
  fit.btcPrice <- Arima(xts_BtcPrice[2:133], order=c(1,2,0), xreg=btc_ts[2:133,c(i)])
...
for (i in 2:ncol(btc_ts)) {
  ...
  fit.btcPrice <- Arima(xts_BtcPrice[3:133], order=c(1,2,0), xreg=cur_xts[3:133])
  summ <- summary(fit.btcPrice)
  newrow <- data.frame(ME = summ[,'ME'],
                       RMSE = summ[,'RMSE'],
                       MAE = summ[,'MAE'],
  ...
  for(j in seq(1, 7, by = 1)) {
    for(k in seq(1, length(regsets), by = 1)) {
      ...
      for (l in 20:1) {
        ...
        if(!is.null(regset)){
          fit.btcPrice.best <- Arima(xts_BtcPrice[start:end], order=c(1,2,0), xreg=regset[start:end,])
          #fit.btcPrice.best <- auto.arima(xts_BtcPrice[start:end], xreg=regset[start:end,])
        }else{
          fit.btcPrice.best <- Arima(xts_BtcPrice[start:end], order=c(1,2,0))
        ...
        print(dates)
        #auto.arima(xts_BtcPrice[start:end], d=1, trace=TRUE, stepwise=FALSE, approx=FALSE, xreg=regset[start:end,])
        #auto.arima(xts_BtcPrice[start:end], d=2, trace=TRUE, stepwise=FALSE, approx=FALSE, xreg=regset[start:end,])
        print("---------------------------------------------------------------")
      }
...
write.csv(profit_res, file = "1_2_0_rolling_without_xreg_diff_3_final.csv",row.names=FALSE)