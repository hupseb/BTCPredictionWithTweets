...
for (i in 2:ncol(btc_ts)) {
  ...
  #fit.btcPrice <- Arima(xts_BtcPrice[2:133], order=c(1,2,1), xreg=diff(btc_ts[1:133,c(i)]))
  fit.btcPrice <- Arima(xts_BtcPrice[2:133], order=c(0,1,0), xreg=btc_ts[2:133,c(i)])
...
for (i in 2:ncol(btc_ts)) {
  ...
  fit.btcPrice <- Arima(xts_BtcPrice[2:133], order=c(0,1,0), xreg=cur_xts[2:133])
  summ <- summary(fit.btcPrice)
  newrow <- data.frame(ME = summ[,'ME'],
                       RMSE = summ[,'RMSE'],
                       MAE = summ[,'MAE'],
...
cur_100 <- stats::lag(xts(btc_ts[,"sp500"], order.by = as.Date(btc_ts[,1])),1)
...
regsets[["Best Technical Predictors lag 1"]] <- data.frame(
  cur_100
)

regsets[["Best Technical Predictors + Best Twitter Sentiment + Best Technical Predictors lag 1"]] <- data.frame(
  cur_100,
  cur_31,
  cur_36
)
...
profit_res <- data.frame(profit_dfs_nr=integer(),
                        ...
                        btc_tec_regs_lag1_only=logical(),
                        twitter_and_btc_regs_and_lag1=logical()
                        )

for(i in seq(50, 120, by = 10)) {
  for(j in seq(1, 7, by = 1)) {
    for(k in seq(1, length(regsets), by = 1)) {
      ...
      for (l in 20:1) {
        ...
        if(!is.null(regset)){
          fit.btcPrice.best <- Arima(xts_BtcPrice[start:end], order=c(0,1,0), xreg=regset[start:end,])
          #fit.btcPrice.best <- auto.arima(xts_BtcPrice[start:end], xreg=regset[start:end,])
        }else{
          fit.btcPrice.best <- Arima(xts_BtcPrice[start:end], order=c(0,1,0))
      ...
      profit_res <- rbind(profit_res, data.frame(profit_dfs_nr=n,
                                                 ...
                                                 btc_tec_regs_lag1_only=regsetname == "Best Technical Predictors lag 1",
                                                 twitter_and_btc_regs_and_lag1=regsetname == "Best Technical Predictors + Best Twitter Sentiment + Best Technical Predictors lag 1"))
...
lmProfit = lm(profit_res$profit ~
              profit_res$arima_size + profit_res$forecast_days +
              profit_res$window_week_start_day + profit_res$twitter_sent_regs +
              profit_res$btc_tec_regs + 
              #interactions
              profit_res$twitter_sent_regs:profit_res$arima_size +
              profit_res$twitter_sent_regs:profit_res$window_week_start_day +
              profit_res$arima_size:profit_res$window_week_start_day +
              profit_res$twitter_sent_regs:profit_res$btc_tec_regs
              )
...
write.csv(profit_res, file = "0_1_0_fix_without_xreg_diff_lag1.csv",row.names=FALSE)
