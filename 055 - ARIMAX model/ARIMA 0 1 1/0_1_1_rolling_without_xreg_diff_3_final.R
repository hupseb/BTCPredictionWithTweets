...
for (i in 2:ncol(btc_ts)) {
  ...
  fit.btcPrice <- Arima(xts_BtcPrice[3:133], order=c(0,1,1), xreg=cur_xts[3:133])
  summ <- summary(fit.btcPrice)
  newrow <- data.frame(ME = summ[,'ME'],
                       RMSE = summ[,'RMSE'],
                       MAE = summ[,'MAE'],
...
write.csv(profit_res, file = "0_1_1_rolling_without_xreg_diff_3_final.csv",row.names=FALSE)
