...
for (i in 2:ncol(btc_ts)) {
  ...
  fit.btcPrice <- Arima(xts_BtcPrice[2:133], order=c(0,1,1), xreg=btc_ts[2:133,c(i)])
...
for (i in 2:ncol(btc_ts)) {
  ...
  fit.btcPrice <- Arima(xts_BtcPrice[2:133], order=c(0,1,1), xreg=cur_xts[2:133])
  summ <- summary(fit.btcPrice)
  newrow <- data.frame(ME = summ[,'ME'],
                       RMSE = summ[,'RMSE'],
                       MAE = summ[,'MAE'],
  ...
  for(j in seq(1, 7, by = 1)) {
    for(k in seq(1, length(regsets), by = 1)) {
      ...
      for (l in 20:1) {
        ...
        if(!is.null(regset)){
          fit.btcPrice.best <- Arima(xts_BtcPrice[start:end], order=c(0,1,1), xreg=regset[start:end,])
          #fit.btcPrice.best <- auto.arima(xts_BtcPrice[start:end], xreg=regset[start:end,])
        }else{
          fit.btcPrice.best <- Arima(xts_BtcPrice[start:end], order=c(0,1,1))
...
write.csv(profit_res, file = "0_1_1_fix_without_xreg_diff.csv",row.names=FALSE)
