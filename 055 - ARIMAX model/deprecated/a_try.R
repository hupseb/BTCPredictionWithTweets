...
cur_101 <- xts(btc_ts[,"sp500"], order.by = as.Date(btc_ts[,1]))
cur_101 <- stats::lag(cur_101,1)
...
for (i in 20:0) {
  fit.btcPrice.best <- auto.arima(xts_BtcPrice[3:(133-i)],stepwise=FALSE, approx=FALSE, xreg=data.frame(diff(cur_31)[3:(133-i)],
                                                                                       diff(cur_35)[3:(133-i)],
                                                                                       diff(cur_101)[3:(133-i)]))
  
  fc_31 <- forecast(auto.arima(diff(cur_31)[3:(133-i)],stepwise=FALSE, approx=FALSE), h=1)
  fc_35 <- forecast(auto.arima(diff(cur_35)[3:(133-i)],stepwise=FALSE, approx=FALSE), h=1)

  #lag backwards
  fc_101 <- xts(btc_ts[,"sp500"], order.by = as.Date(btc_ts[,1]))
  fc_101 <- diff(fc_101)
  fc_101 <- fc_101[133-i]
...
for (i in 20:0) {
  fit.btcPrice <- Arima(xts_BtcPrice[3:(133-i)], order=c(2,1,2))
  #summary(fit.btcPrice)
  fc <- forecast(fit.btcPrice, h=1)
  #print(fc)
  print(fc[["mean"]][1])
