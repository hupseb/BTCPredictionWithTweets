  ...
  for(j in seq(1, 7, by = 1)) {
    for(k in seq(1, length(regsets), by = 1)) {
      ...
      for (l in 20:1) {
        ...
        if(!is.null(regset)){
          fit.btcPrice.best <- Arima(xts_BtcPrice[start:end], order=c(2,1,2), xreg=regset[start:end,])
        }else{
        ...
        print(index(xts_BtcPrice[start,]))
        print(index(xts_BtcPrice[end,]))
...
colnames(profit_res) <- c("profit_dfs_nr",
                          "Profit",
                          "ARIMA Size",
                          "Forecast Days",
                          "Window Week Start Day",
                          "First Forecast Date",
                          "Last Forecast Date",
                          "Twitter Sentiment Regressors",
                          "BTC Technical Regressors")

#maybe save results to database
#...

#TODO
#... name columns of regsets correctly
#... name columns of profit_dfs correctly
#... run ANOVA for profit table - SEE SAVED LINK IN CHROME
