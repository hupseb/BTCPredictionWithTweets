...
for (i in 2:ncol(btc_ts)) {
  ...
  fit.btcPrice <- Arima(xts_BtcPrice[2:133], order=c(2,2,0), xreg=btc_ts[2:133,c(i)])
...
for (i in 2:ncol(btc_ts)) {
  ...
  fit.btcPrice <- Arima(xts_BtcPrice[2:133], order=c(2,2,0), xreg=cur_xts[2:133])
  summ <- summary(fit.btcPrice)
  newrow <- data.frame(ME = summ[,'ME'],
                       RMSE = summ[,'RMSE'],
                       MAE = summ[,'MAE'],
...
write.csv(profit_res, file = "2_2_0_fix_without_xreg_diff.csv",row.names=FALSE)
